...
### (NEW) TC0 Measurement Code

To run the TC0 measurement code, perform the same steps as above, but instead perform `git checkout tc0` instead of `git checkout k4`.

The measurement code measures the stopping distance as well as the average time to go around the outer most loop. Sensor `E8` is used to measure the loop time. Whenever the train passes over `E8`, the loop time is recorded. 9 loop times are recorded in total before switch 11 is set to straight and the train is set to speed 0. This allows for the stopping distance to be measured as well.

The measurement results are located in `docs/tc0/tc0.csv`
