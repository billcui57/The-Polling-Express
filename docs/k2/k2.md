...
Now $A$ is still blocked but $B$ is free to run. Later on when another task executes `Reply` to $A$, `msg_copy` is executed to copy that task's message back to $A$, both tasks then get placed back onto the ready queue.
...
Nameserver's overarching datastructure is a hashtable. Since there are no syscalls for "deregistering" a TCB from a name, the hashtable only has get and insert operations. The hashtable uses a singular modulos hash function that is unit tested in `/tests/unit/hashtable.c`. 
...
The user task `task_k2rpsinit` creates the nameserver, the RPSserver, as well as two players. Be sure to add it to the schedular in `main.c` if you wish to test out RPS. One bot is set to quit after 50 moves.


### (NEW) RPS Program Output Explained

Upon executing the user task, both the player and the bot calls `SignUp`. The first game, which is game 0, is replied to both players. 
...
[K2 UPDATE] the `MAX_NUM_TASKS` is now set to be 1000 to faciliate for multiple RPS games.
