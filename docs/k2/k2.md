...
The user task `task_k2rpsinit` creates the nameserver, the RPSserver, as well as two players. Be sure to add it to the schedular in `main.c` if you wish to test out RPS. One player is user controlled, the other is a bot. The bot is set to quit after 10 moves. This allows for testing of various quit scenarios.
